...
2. **Writing guide update** — 3 new/modified sections in `docs/writing-guide-blog-b.md`
...
- Target keyword: "다이소 방향제" must remain in title, Block 1, Block 10, tags
...
| Block | Act | Tension | 심리 훅 | Open Loop | Purpose |
|-------|-----|---------|---------|-----------|---------|
| 1 (text) | Hook | 4 | 정당화 | YES — "그래서 하나 사왔는데..." | 공감 + 키워드 삽입 |
| 2 (image) | Hook | — | — | — | 시각 증거 |
| 3 (text) | 시도 | 5 | 구체적 장면 | YES — 결론 없이 장면만 | 디테일 신뢰 |
| 4 (text) | 시도 반응 | 5 | 낙차 세팅 | YES — "됐다 싶었는데..." | 실패 대비용 긍정 |
| 5 (text) | 실패 | **8** (peak) | 공포 | YES — "다른 거 없나 찾아봤거든요" | 텐션 피크 |
| 6 (image) | 실패 | — | — | — | 시각 증거 |
| 7 (text) | 발견 | **7** | 정보갭 | YES — "남편한테 보여줬더니..." | 발견 감정 |
| 8 (image) | 발견 | — | — | — | 시각 증거 |
| 9 (text) | 만족 | 4 | 감정 해소 | NO | 릴리즈 시작 |
| 10 (text) | 정리 | 2 | SEO wrap | NO | 릴리즈 완료 |

**Key principle:** Blocks 1-7 end with open loops. Blocks 9-10 close.
...
- "잠깐, 선이 없네?" style reaction — one surprise point only
- Move remaining features (번호판 겸용 etc.) to Block 9
- End with open loop: "남편한테 보여줬더니..."
- 3 sentences (shorter, punchier than current)
...
Replace with rewritten B-004 text. Each block annotation adds:
- **TENSION:** score (e.g., 8)
- **HOOK:** type (e.g., 공포)
- **LOOP:** open loop technique used (e.g., 복선)
