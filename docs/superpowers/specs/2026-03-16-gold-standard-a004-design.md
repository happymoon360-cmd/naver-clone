...
- **commentsVisible / commentsOpen:** omit — defaults handled by `normalizePost`
...
**Implementation note:** The current live A-004 entry has only 4 blocks (`a004-1`, `a004-3`, `a004-4`, `a004-6`). **Delete the entire existing `content` array and replace it with the 10 new blocks below.** Do not patch or append — full replacement only.
...
- **TENSION:** 6 — 기대감 형성 후 "나쁘지 않았습니다" (과거형) → 독자가 "그런데 왜 바꿨나?" 질문 생김. Open Loop: 과거형 종결로 미완결 암시.
- **Length:** 2~3 문장
...
- **TENSION:** 8 — 최저점. 문제가 명확해짐 → 독자가 "그럼 어떻게 했나"에 집중
- **Length:** 3~4 문장
...
| # | id | author | content | parentId | likes | isLiked | timestamp | purpose |
|---|-----|--------|---------|----------|-------|---------|-----------|---------|
| 1 | 1 | g90주인장 | "다이소 방향제 저도 써봤는데 딱 일주일이더군요." | — | 4 | false | `Date.now() - 8 * 86400000 + 3 * 3600000` | 공감 + credibility |
| 2 | 2 | 깔끔한드라이버 | "맞습니다. 처음엔 괜찮았는데 금방 퍼지더군요." | 1 | 1 | false | `Date.now() - 8 * 86400000 + 5 * 3600000` | 블로거 호응 (링크 없음) |
| 3 | 3 | GV80입문자 | "지금 쓰시는 거 어디서 구하셨나요? 선 없는 타입 맞나요?" | — | 2 | false | `Date.now() - 8 * 86400000 + 27 * 3600000` | 자연스러운 구매 질문 |
| 4 | 4 | 깔끔한드라이버 | "여기서 봤습니다. https://sjhur3601.cafe24.com/ 태양열이라 선 연결 없습니다." | 3 | 1 | false | `Date.now() - 8 * 86400000 + 30 * 3600000` | **유일한 링크** |
| 5 | 5 | 보배드림러 | "번호판 겸용이라는 게 신기하네요. 저도 따로 두는 거 정리하려고 했는데." | — | 2 | false | `Date.now() - 8 * 86400000 + 52 * 3600000` | 잡담 + 사회적 증거 |
...
**Format (with filled example for Block 1):**
```
[Block 1 — Hook]
"G80 출고하고 나서 제일 먼저 한 게 다이소 방향제 달기였습니다. 새 차 냄새가 아직 남아 있긴 했는데, 이게 금방 빠진다는 걸 알았거든요. 값도 부담 없고 바로 살 수 있어서 일단 하나 집어왔습니다."
  ← WHY: G80 차종 명시로 동질감 훅. "나와 같은 상황인 사람"으로 진입. 남성 독자는 구체적 상황에서만 읽기 시작함.
  ← TONE: 합니다체. 감탄 없음. "일단 하나 집어왔습니다" = 보배드림 첫 문장 패턴. 과잉 없음.
  ← CREDIBILITY: 구체적 맥락(출고 직후 + 이유 명시) = 광고 아닌 실제 경험처럼 읽힘.
  ← TENSION: 3 — 낮은 시작. 문제 없음. "다이소 방향제" 키워드 자연 삽입. Open Loop: "그런데..."는 없음. 다음 블록으로 자연히 이어짐.
```

모든 10개 블록을 이 형식으로 작성한다. 각 블록당 4개 레이블 모두 필수.

**Sequencing:** Write A-004 post text first (Deliverable 1), then copy final text into Section 6 with inline annotations for all 10 blocks (Deliverable 2). Section 6 content is **agent-generated output** — it is not pre-filled in this spec. The implementing agent writes the actual post text and then annotates it. Section 6 must be fully filled in by the agent — no placeholder text in the final `docs/writing-guide-blog-a.md`.
...
- Other Blog A posts (A-001~A-003, A-005~A-006) are NOT modified in this task. Note: these posts currently have 2 Cafe24 links in their comments — fixing them is out of scope here and will be addressed when those posts are rewritten.
